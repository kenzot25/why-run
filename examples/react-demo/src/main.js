...
const fetchUser = trace("fetchUser", async (id) => {
    await new Promise((resolve) => setTimeout(resolve, 100));
    return { id, name: `User ${id}` };
});
const updateState = trace("updateState", (setUser, user) => {
    setUser(user);
});
function App() {
    ...
    const handleFetch = useTracedCallback("handleFetch", async () => {
        if (!userId)
            return;
        setLoading(true);
    ...
    return (<div style={{ padding: 40, fontFamily: "system-ui" }}>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
        <input type="number" value={userId} onChange={(e) => setUserId(e.target.value)} placeholder="Enter user ID" style={{ padding: "8px 12px", fontSize: 16 }}/>
        <button onClick={handleFetch} disabled={loading} style={{ padding: "8px 16px", fontSize: 16 }}>
          {loading ? "Loading..." : "Fetch User"}
        </button>
      </div>

      {user && (<div style={{ marginTop: 20, padding: 20, background: "#f5f5f5", borderRadius: 8 }}>
          <h3>User Data</h3>
